...
- **ui-trajectory stays deferred in the official Web application** (a developer inspection surface, terminology-dense). The local Electron application applies a separately owned [Chinese presentation layer](../feature/2026-08-14-electron-trajectory-localization.md) without changing this package-level locale decision.
...
- ui-primitives' Chinese defaults still render Chinese under the English locale **until a consumer passes labels** — the upstream JsonTree consumer in ui-trajectory remains unmigrated and shows its English defaults; the Electron-only presentation layer translates that rendered chrome independently.
