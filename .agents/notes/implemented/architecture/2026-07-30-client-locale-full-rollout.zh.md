...
- **官方 Web 应用中的 ui-trajectory 仍整包缓做**（开发者检查面，术语密集）。本地 Electron 应用采用单独归属的[中文呈现层](../feature/2026-08-14-electron-trajectory-localization.md)，不改变这项包级 locale 决策。
...
- ui-primitives 的中文默认值在英文语言下依旧是中文，**直到消费方传入 labels**——ui-trajectory 中的上游 JsonTree 消费方仍未迁移并显示英文默认值；Electron 专属呈现层会单独翻译这部分已渲染界面。
